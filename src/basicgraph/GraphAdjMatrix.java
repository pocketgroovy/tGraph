...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		// XXX Implement this method in week 1
		 ...
		 int[][] mat2 = matrixMultiplication(adjMatrix);
	...
	private int[][] matrixMultiplication(int[][] admat){
		int[][] result = new int[admat.length][admat.length];
		for(int i = 0; i < admat.length; i++){
			for(int j = 0; j <admat[0].length;j++){
				for(int k = 0; k < admat.length; k++){
					result[i][j] += admat[i][k] * admat[k][j];
				}
			}
		}
		return result;
